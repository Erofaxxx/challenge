#!/bin/bash

# Скрипт для запуска обучения в tmux сессии
# Можно выйти из терминала - обучение продолжится

SESSION_NAME="mamba_training"

# Проверяем установлен ли tmux
if ! command -v tmux &> /dev/null; then
    echo "❌ ОШИБКА: tmux не установлен!"
    echo "Установите: sudo apt-get install tmux"
    exit 1
fi

# Проверяем существует ли уже сессия
if tmux has-session -t $SESSION_NAME 2>/dev/null; then
    echo "⚠️  Сессия '$SESSION_NAME' уже существует!"
    echo ""
    echo "Выберите действие:"
    echo "  1) Подключиться к существующей сессии"
    echo "  2) Удалить старую и создать новую"
    echo "  3) Отмена"
    echo ""
    read -p "Ваш выбор (1/2/3): " choice

    case $choice in
        1)
            echo "Подключаемся к сессии..."
            tmux attach-session -t $SESSION_NAME
            exit 0
            ;;
        2)
            echo "Удаляем старую сессию..."
            tmux kill-session -t $SESSION_NAME
            ;;
        *)
            echo "Отмена."
            exit 0
            ;;
    esac
fi

echo "======================================"
echo "Запуск обучения в tmux"
echo "======================================"
echo ""
echo "Параметры по умолчанию:"
echo "  • Количество моделей: 3"
echo "  • Эпохи: 15"
echo "  • Batch size: 64"
echo "  • d_model: 128"
echo ""
echo "Запускаем обучение..."
echo ""

# Создаем новую tmux сессию
tmux new-session -d -s $SESSION_NAME -n "training"

# Переходим в директорию и запускаем обучение
tmux send-keys -t $SESSION_NAME "cd $(pwd)" C-m
tmux send-keys -t $SESSION_NAME "echo '===================================='" C-m
tmux send-keys -t $SESSION_NAME "echo 'ОБУЧЕНИЕ MAMBA-2 МОДЕЛЕЙ'" C-m
tmux send-keys -t $SESSION_NAME "echo '===================================='" C-m
tmux send-keys -t $SESSION_NAME "echo ''" C-m
tmux send-keys -t $SESSION_NAME "echo 'Сессия: $SESSION_NAME'" C-m
tmux send-keys -t $SESSION_NAME "echo 'Время старта: \$(date)'" C-m
tmux send-keys -t $SESSION_NAME "echo ''" C-m
tmux send-keys -t $SESSION_NAME "python train.py --data ../datasets/train.parquet 2>&1 | tee training.log" C-m

echo "✅ Обучение запущено в tmux сессии '$SESSION_NAME'"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "📋 Как работать с tmux:"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "1. 👀 Подключиться к сессии (смотреть прогресс):"
echo "   tmux attach-session -t $SESSION_NAME"
echo ""
echo "2. 🚪 Выйти из сессии (обучение продолжится):"
echo "   Нажмите: Ctrl+B, затем D"
echo ""
echo "3. 📊 Посмотреть логи без подключения:"
echo "   tail -f training.log"
echo ""
echo "4. 📋 Список всех tmux сессий:"
echo "   tmux ls"
echo ""
echo "5. ⏹️  Остановить обучение:"
echo "   tmux kill-session -t $SESSION_NAME"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
echo "💡 Совет: Можете закрыть терминал - обучение продолжится!"
echo ""
echo "Подключаюсь к сессии через 3 секунды..."
sleep 3

# Подключаемся к сессии
tmux attach-session -t $SESSION_NAME
